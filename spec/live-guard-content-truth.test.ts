...
describe('live guard content truth', () => {
  it('H1 exempts untracked cache output but still reverts and counts tracked writes', { timeout: 15_000 }, async () => {
    const trackedSource = seedProject();
    const trackedPyc = join(projectDir, 'pkg', '__pycache__', 'tracked.pyc');
    mkdirSync(dirname(trackedPyc), { recursive: true });
    writeFileSync(trackedPyc, 'tracked bytecode preimage\n');
    commitProject();
    const cachePath = join(projectDir, 'pkg', '__pycache__', 'module.cpython-312.pyc');
    ...
    const result = await run([], 'cache-exemption', { async run(_prompt, _role, opts) {
      ...
      if (invocationCount === 1) {
        ...
        writeFileSync(cachePath, 'regenerated bytecode\n');
        writeFileSync(trackedSource, 'unauthorized source mutation\n');
        writeFileSync(trackedPyc, 'unauthorized tracked bytecode mutation\n');
        const deadline = Date.now() + 3_000;
        while (Date.now() < deadline && (
          readFileSync(trackedSource, 'utf8') !== 'original bytes\n'
          || readFileSync(trackedPyc, 'utf8') !== 'tracked bytecode preimage\n'
        )) {
    ...
    expect(readFileSync(cachePath, 'utf8')).toBe('regenerated bytecode\n');
    expect(readFileSync(trackedSource, 'utf8')).toBe('original bytes\n');
    expect(readFileSync(trackedPyc, 'utf8')).toBe('tracked bytecode preimage\n');
    expect(result.incidents.map((incident) => incident.path).sort()).toEqual([
      'pkg/__pycache__/tracked.pyc',
      'tracked.txt',
    ]);
    const events = readRunEvents(projectDir, result.final.runId);
    expect(events.filter((event) => event.type === 'live_constraint_exemptions')).toEqual([
      expect.objectContaining({ stageId: 'scout', invocationIndex: 1, exemptedCount: 1 }),
    ]);
    expect(events.filter((event) => event.type === 'live_constraint_violation')).toHaveLength(2);
  });
