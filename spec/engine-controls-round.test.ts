...
function oneStageWorkflow(name: string, prompt = 'exercise the engine control'): { config: WorkflowConfig; yaml: string } {
  return {
    yaml: [
      `name: ${name}`,
      'defaults:',
...
describe('engine controls round replays', () => {
  ...
  it('2 exempts every frozen untracked Vitest member and preserves old-way controls', { timeout: 20_000 }, async () => {
    ...
    writeJson(join(projectDir, 'package-lock.json'), {});
    writeJson(join(projectDir, 'tsconfig.json'), {
      compilerOptions: { outDir: 'dist', tsBuildInfoFile: '.cache/tsc.tsbuildinfo' },
    });
    const generatedWithPreimagePath = 'dist/.flowcrew-build-manifest.json';
    write(join(projectDir, generatedWithPreimagePath), '{"generation":"before"}\n');
    const declared = oneStageWorkflow('vitest-cache-after', 'Run npm run build and npm test before finishing.');
    ...
    const adapter: Adapter = {
      async run(_prompt, _role, options) {
        ...
        options.onCommandLifecycle?.({
          phase: 'started', id: commandId, command: 'npm run build', timestamp: new Date().toISOString(),
        });
        write(join(projectDir, cachePath), '{"files":[]}\n');
        write(join(projectDir, knownExemptPath), 'generated bytecode\n');
        write(join(projectDir, generatedWithPreimagePath), '{"generation":"after"}\n');
        options.onCommandLifecycle?.({
    ...
    expect(exemptions).toEqual([expect.objectContaining({ exemptedCount: 3 })]);
    expect(violations).toEqual([]);
    expect(existsSync(join(projectDir, cachePath))).toBe(true);
    expect(readFileSync(join(projectDir, generatedWithPreimagePath), 'utf8')).toBe('{"generation":"after"}\n');
    const generatedAudit = readJson(join(created.runDirPath, 'stages', 'subject', 'constraint_audit_attempt_1.json'));
    expect(generatedAudit.rolledBackWrites).not.toContain(generatedWithPreimagePath);
    ...
    const authoredProjectDir = join(sandboxRoot, 'problem-2', 'authored-generated-control', 'project');
    const authoredAgentsDir = seedEngineProject(authoredProjectDir);
    writeJson(join(authoredProjectDir, 'package.json'), {
      scripts: { build: 'tsc -p tsconfig.json', test: 'vitest run' },
      devDependencies: { vitest: 'fixture' },
    });
    writeJson(join(authoredProjectDir, 'package-lock.json'), {});
    writeJson(join(authoredProjectDir, 'tsconfig.json'), { compilerOptions: { outDir: 'dist' } });
    const authoredDeclared = oneStageWorkflow('authored-generated-control', 'Author the requested manifest and finish.');
    const authoredCreated = prepareRun(authoredProjectDir, authoredDeclared);
    const authoredPath = 'dist/authored-by-stage.json';
    ...
    const authoredFinal = await runWorkflow(
      authoredDeclared.config, authoredDeclared.yaml, authoredProjectDir, authoredAdapter, new Map(), undefined,
      authoredAgentsDir, authoredCreated.runId, 'Authored generated-looking path control', true, false,
    );
    const authoredEvents = readRunEvents(authoredProjectDir, authoredCreated.runId)
      .filter((event) => event.type === 'live_constraint_violation');
    expect(authoredFinal.status).toBe('failed');
    expect(authoredInvocationCount).toBe(2);
    expect(authoredEvents).toHaveLength(2);
    expect(authoredEvents.every((event) => event.files?.includes(authoredPath))).toBe(true);
    expect(existsSync(join(authoredProjectDir, authoredPath))).toBe(false);
    ...
    afterEvidence(2, {
      ...
      authoredGeneratedPathControl: {
        id: 'authored-generated-looking-write-without-validation-command',
        disposition: 'violation-then-failed-on-repeat',
        differsBy: ['no configured command lifecycle', 'authored path rather than command by-product'],
        engineRecordedOutput: {
          finalStatus: authoredFinal.status,
          invocationCount: authoredInvocationCount,
    ...
    for (const variant of variants) {
      ...
      const result = await internals.act({
        ...
        rows: [{
          ...
          authority: 'action',
          text: 'continue the same implementation direction',
        }],
      }], ['unaccused', {
        version: 1,
        stageId: 'unaccused',
