...
export default function TaskSettingsPanel({ task, open, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    fetchSettings().then(s => {
      if (s.default_timeout_ms) setTimeoutMin(Math.round(s.default_timeout_ms / 60000));
    }).catch(() => {});
  }, []);

  useEffect(() => {
    if (!task) return;
    if (task.maxIterations) setMaxIterations(task.maxIterations);
    if (task.autoApproveRetries !== undefined) setAutoApprove(task.autoApproveRetries);
  }, [task]);
    ...
    <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
      ...
      <div className="relative w-80 h-full bg-rc-bg border-l border-rc-border shadow-xl overflow-auto p-5 space-y-4" onClick={e => e.stopPropagation()}>
        ...
        <label className="block text-xs text-rc-text-secondary">
          ⏱ Stage timeout (min)
          <input type="number" min={1} value={timeoutMin} onChange={e => setTimeoutMin(+e.target.value)} disabled={isReadOnly}
        ...
        <label className="block text-xs text-rc-text-secondary">
          🔄 Max iterations
          <input type="number" min={1} value={maxIterations} onChange={e => setMaxIterations(+e.target.value)} disabled={isReadOnly}
            className="w-full mt-1 px-2 py-1 bg-rc-code border border-rc-border rounded-input text-sm text-rc-text focus:outline-none focus:border-rc-accent disabled:opacity-50" />
        </label>
