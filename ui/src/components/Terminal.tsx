...
const Terminal = forwardRef<TerminalHandle, TerminalProps>(({ wsUrl, staticContent, className, interactive, onReady, onPlanReady, onBriefNotReady }, ref) => {
  ...
  useEffect(() => {
    ...
    if (wsUrl) {
      ...
      ws.onopen = () => {
        // Send resize FIRST so server can spawn PTY with correct dimensions
      ...
      if (interactive) {
        ...
        function flushInput() {
          if (!inputBuf || ws.readyState !== WebSocket.OPEN) { inputBuf = ''; return; }
          const data = inputBuf;
          inputBuf = '';
          // If accumulated data is longer than a few chars, it's likely a paste — bracket it
        ...
        term.onData((data) => {
          // Single char (normal typing) or escape sequence (arrow keys) — send immediately
          if (data.length <= 1 || data.startsWith('\x1b')) {
            if (inputBuf) flushInput(); // flush any pending paste first
            if (ws.readyState === WebSocket.OPEN) sendBinary(ws, 0x00, data);
            return;
          }
          // Multi-char data (paste) — accumulate and debounce
          inputBuf += data;
          if (inputTimer) clearTimeout(inputTimer);
          inputTimer = setTimeout(flushInput, 50);
        });

        // Explicit Ctrl+V/Cmd+V paste — always bracket
