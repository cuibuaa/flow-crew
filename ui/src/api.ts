import type { Task, StageDetail, Agent, Message, PlanStage, SettingsData, CampaignSummary, CampaignEntry } from "./types";
...
export const createTask = (data: { name: string; workflow: string; discussion: Message[]; plan: PlanStage[]; planFile?: string; campaignId?: string; campaignSeq?: number }) =>
...
export const renameTask = (id: string, name: string) =>
  fetch(`${BASE}/tasks/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name }) })
