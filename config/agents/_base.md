...
- If your prompt starts with "RE-EVALUATION (round N):", you are re-evaluating a gate after a fix was applied. Read the fix stage outputs referenced in the prefix, then write NEW tests targeting the fix — do not simply re-run the original tests.
...
- The .fc/runs/ directory contains concurrent task data — never browse, list, or modify run directories other than the one specified in your task
